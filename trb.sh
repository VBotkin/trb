#!/bin/sh
# trb: Read the Textus Receptus from your terminal
...
while [ $# -gt 0 ]; do
	...
	if [ "$1" = "--" ]; then
		...
		get_data trb.tsv | awk -v cmd=list "$(get_data trb.awk)"
		exit
	elif [ "$1" = "-W" ]; then
...
if [ $# -eq 0 ]; then
	...
	while true; do
		...
		get_data trb.tsv | awk -v cmd=ref -v ref="$ref" "$(get_data trb.awk)" | ${PAGER}

	done
	exit 0
fi

get_data trb.tsv | awk -v cmd=ref -v ref="$*" "$(get_data trb.awk)" | ${PAGER}
